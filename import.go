...
type (
	...
	File        = loader.File
	PackagePath = loader.PackagePath
)
...
func Optimize(
	...
	Clean(l, f)
	Sort(f.File, projectPkgPrefix, companyPkgPrefix)
}

func Uses(f *File, pkg *types.Package) (use bool) {
	path := pkg.Path()
	s := ImportSpec(f.File, path)
	importedName := importName(s, pkg.Name())
	if staticUsesImport(f.File, s, importedName) == used {
		return true
	}

	usePkg := func(id *ast.Ident) bool {
		switch obj := f.Pkg.TypesInfo.ObjectOf(id).(type) {
		case nil:
			return false
		case *types.PkgName:
