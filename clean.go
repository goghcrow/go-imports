...
func Clean(l *loader.Loader, file *File) {
	i := &importCleaner{
		loader:    l,
		file:      file,
...
func (i *importCleaner) imports() {
	walkImportGroup(i.file.File, func(d *ast.GenDecl) {
		for _, spec := range d.Specs {
			s := spec.(*ast.ImportSpec)
			path := SpecPath(s)
			pkg, ok := i.loader.All[path]
...
func (i *importCleaner) uses() {
	walkImportGroup(i.file.File, func(d *ast.GenDecl) {
		for _, spec := range d.Specs {
			...
			if staticUsesImport(i.file.File, s, importedName) == used {
				i.useMap[path] = true
			}
		}
	})
	...
	astutil.Apply(i.file.File, nil, func(c *astutil.Cursor) bool {
		// skip import spec
...
func (i *importCleaner) markUses(id *ast.Ident) {
	...
	switch obj := i.file.Pkg.TypesInfo.ObjectOf(id).(type) {
	case nil:
		return
...
func (i *importCleaner) clearEmptyDecl() {
	xs := make([]ast.Decl, 0, len(i.file.File.Decls))
	for _, decl := range i.file.File.Decls {
		d, ok := decl.(*ast.GenDecl)
		if !ok || d.Tok != token.IMPORT || len(d.Specs) > 0 {
			xs = append(xs, decl)
		}
	}
	i.file.File.Decls = xs
}

func (i *importCleaner) tryGetPkgName(path string) string {
	pkg := i.loader.All[path]
	if pkg != nil {
		return pkg.Name
	} else {
		// guess
